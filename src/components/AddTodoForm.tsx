...
import { Category, Priority, SubTodo, AlarmRepeatType, AlarmSoundType } from '../types';
...
export default function AddTodoForm({ onAddTodo, prefilledDate, onClearPrefilledDate }: AddTodoFormProps) {
  ...
  useEffect(() => {
    if (prefilledDate) {
      ...
      setIsOpen(true);
      setTimeout(() => {
        const el = document.getElementById('add-todo-wrapper');
        if (el) {
  ...
  const [alarmRepeatInterval, setAlarmRepeatInterval] = useState<number>(30); // Default to 30 e.g. 30분마다
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    setAlarmRepeat('none');
    setAlarmRepeatDays([]);
    setAlarmRepeatInterval(30);
    setIsMonthlyDayPickerConfirmed(false);
    setAlarmSound('zen_bell');
  ...
  return (
    <div className="w-full" id="add-todo-wrapper">
      {/* 3D Glass Trigger Button */}
      <div className="flex justify-center mb-6">
        <motion.button
          onClick={() => setIsOpen(!isOpen)}
          className="relative z-10 flex items-center gap-2 overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-400 via-teal-350 to-green-400 px-6 py-3.5 text-base font-bold text-white shadow-[0_6px_20px_rgba(52,211,153,0.25)] transition-all duration-300 [transform-style:preserve-3d] [perspective:500px] hover:translate-y-[-2px] hover:shadow-[0_8px_25px_rgba(52,211,153,0.35)] active:translate-y-[1px]"
          whileTap={{ scale: 0.98 }}
        ...
        >
          ...
          <span>{isOpen ? '닫기' : '새로운 목표 추가하기'}</span>
          <Sparkles size={16} className="text-emerald-200 animate-pulse ml-1" />
        </motion.button>
      </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20, scale: 0.98, rotateX: -5 }}
            transition={{ duration: 0.35, ease: 'easeOut' }}
            className="w-full overflow-hidden rounded-3xl border border-white/60 bg-white/50 p-6 shadow-xl backdrop-blur-2xl [transform-style:preserve-3d] [perspective:1000px] max-w-2xl mx-auto mb-8 relative"
            id="add-todo-form-panel"
          >
            {/* Soft decorative glow spots inside the glass container */}
            <div className="absolute bottom-0 left-0 -ml-12 -mb-12 h-32 w-32 rounded-full bg-emerald-500/10 blur-2xl" />
            <div className="absolute top-0 right-0 -mr-12 -mt-12 h-32 w-32 rounded-full bg-cyan-500/10 blur-2xl" />

            <form onSubmit={handleSubmit} className="relative z-10 flex flex-col gap-5">
              <h2 className="text-xl font-bold tracking-tight text-slate-900 mb-1 flex items-center gap-2">
                <span className="h-2.5 w-2.5 rounded-full bg-emerald-600 shadow-[0_0_8px_rgba(16,185,129,0.3)]" />
                새로운 3D 작업 작성
              </h2>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold tracking-wider text-slate-650 uppercase">계획 제목 *</label>
                  <button
                    type="button"
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder={isTitleListening ? "음성을 듣고 있어요. 말씀해 주세요..." : "무엇을 이뤄내실 건가요?"}
                    className="w-full rounded-2xl border border-slate-200/80 bg-white/85 px-4 py-3 pr-4 text-slate-850 placeholder-slate-400 outline-none transition-all duration-300 focus:border-emerald-400 focus:bg-white focus:ring-1 focus:ring-emerald-500/10 font-sans shadow-sm"
                    id="todo-input-title"
                  />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold tracking-wider text-slate-650 uppercase">세부 내용 (선택)</label>
                  <button
                    type="button"
                ...
                <div className="relative">
                  <textarea
                    ...
                    placeholder={isDescListening ? "음성을 듣고 있어요. 말씀해 주세요..." : "목표 달성을 위한 간단한 설명을 적어보세요..."}
                    rows={2}
                    className="w-full rounded-2xl border border-slate-200/80 bg-white/85 px-4 py-3 pr-4 text-slate-800 placeholder-slate-400 outline-none transition-all duration-300 focus:border-emerald-400 focus:bg-white focus:ring-1 focus:ring-emerald-500/10 font-sans resize-none shadow-sm"
              ...
              {/* Category selector */}
              <div className="flex flex-col gap-2">
                <label className="text-xs font-bold tracking-wider text-slate-650 uppercase">카테고리</label>
                <div className="flex flex-wrap gap-2">
                  {(['Work', 'Personal', 'Shopping', 'Creative', 'Health'] as Exclude<Category, 'All'>[]).map((cat) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-2 rounded-xl px-3.5 py-2 text-xs font-medium border transition-all duration-300 [transform-style:preserve-3d] ${
                          isSelected
                            ? 'bg-emerald-500 border-transparent text-white shadow-md shadow-emerald-500/20'
                            : 'bg-white/60 border-slate-200/70 text-slate-600 hover:bg-white/80 hover:text-slate-900 hover:border-slate-350'
                        }`}
                        id={`add-cat-choice-${cat.toLowerCase()}`}
                      >
                        <Icon size={14} className={isSelected ? 'text-emerald-100' : 'text-slate-500'} />
              ...
              {/* Priority & Due Date (Two columns on desktop) */}
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {/* Priority Selector */}
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-bold tracking-wider text-slate-650 uppercase">우선순위</label>
                  <div className="grid grid-cols-3 gap-2">
                    {(['low', 'medium', 'high'] as Priority[]).map((p) => {
                      const isSelected = priority === p;
                      const activeColors = {
                        low: 'hover:border-emerald-500/30 bg-emerald-100 text-emerald-800 border-emerald-300 font-extrabold shadow-sm',
                        medium: 'hover:border-amber-500/30 bg-amber-100 text-amber-850 border-amber-300 font-extrabold shadow-sm',
                        high: 'hover:border-rose-500/30 bg-rose-100 text-rose-850 border-rose-300 font-extrabold shadow-sm',
                      };
                      const defaultColors = {
                        low: 'bg-white/60 border-slate-200/70 text-slate-500 hover:bg-emerald-50 hover:border-emerald-250 hover:text-emerald-850',
                        medium: 'bg-white/60 border-slate-200/70 text-slate-500 hover:bg-amber-50 hover:border-amber-250 hover:text-amber-850',
                        high: 'bg-white/60 border-slate-200/70 text-slate-500 hover:bg-rose-50 hover:border-rose-250 hover:text-rose-850',
                      };
                      ...
                      return (
                        <button
                          ...
                          className={`rounded-xl border py-2 text-xs text-center transition-all duration-300 font-bold ${
                            isSelected ? activeColors[p] : defaultColors[p]
                          }`}
                          id={`add-priority-choice-${p}`}
                        >
                ...
                {/* Due Date */}
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-bold tracking-wider text-slate-650 uppercase">마감 기한</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border border-slate-200 bg-white py-2.5 pl-11 pr-4 text-slate-800 outline-none transition-all duration-300 focus:border-emerald-400 focus:bg-white focus:ring-1 focus:ring-emerald-500/10 font-mono text-sm [color-scheme:light]"
                      id="todo-input-duedate"
                    />
                  </div>
                </div>
              </div>

              {/* Alarm & Google Calendar Sync integration section */}
              <div className="flex flex-col gap-4 border-t border-slate-200/50 pt-4" id="alarm-calendar-wrapper-container">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2" id="alarm-calendar-wrapper">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    >
                      ...
                      <span className={`text-[10px] uppercase font-bold py-1 px-2.5 rounded-xl transition-all duration-300 ${
                        alarmTime && isAlarmConfirmed
                    ...
                    <AnimatePresence>
                      {isAlarmModalOpen && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" id="alarm-picker-overlay">
                          {/* Backdrop */}
                          <motion.div
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                          ...
                          >
                              ...
                              <div className="flex flex-col gap-1.5 text-left">
                                ...
                                <div className="flex gap-2 items-center justify-between p-3.5 rounded-2xl bg-slate-50 border border-slate-100 shadow-inner">
                                  ...
                                  <select
                                    ...
                                    className="flex-1 rounded-xl border border-slate-200 bg-white px-2 py-2 text-xs font-mono font-bold text-slate-800 outline-none focus:border-emerald-500 shadow-sm transition-colors cursor-pointer"
                                    id="add-alarm-hour"
                                  >
                                ...
                                <button
                                  ...
                                  id="direct-time-select-confirm-btn"
                                >
                            ...
                            {/* Confirmation Footer inside the picking screen */}
                            <div className="mt-5 flex gap-2">
                              <button
                                ...
                                className="flex-1 rounded-2xl border border-slate-200 bg-slate-50 py-3 text-xs font-bold text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition-all duration-300"
                                id="alarm-picker-clear-btn"
                              >
                                비우기
                              </button>
                              <button
                  ...
                  <div className="flex flex-col justify-end pb-1.5">
                    <button
                      ...
                      className={`flex items-center justify-between w-full rounded-2xl border p-3.5 transition-all duration-300 ${
                        syncToCalendar
                          ? 'bg-gradient-to-br from-emerald-600 via-teal-850 to-slate-900 border-transparent text-white shadow-md'
                          : 'bg-white/60 border-slate-200 text-slate-600 hover:bg-white/80'
                      }`}
                      id="add-todo-calendar-sync-toggle"
                    >
                      <div className="flex items-center gap-2">
                        <CalendarCheck size={16} className={syncToCalendar ? 'text-emerald-300' : 'text-slate-500'} />
                        <div className="text-left">
                          <div className="text-xs font-bold">Google Calendar 연동</div>
                          <div className={`text-[9px] ${syncToCalendar ? 'text-emerald-200' : 'text-slate-400'}`}>계획 작성 시 Google Calendar에 자동 예약</div>
                        </div>
                      </div>
                      {/* Toggle pill */}
                      <div className={`w-8 h-4.5 rounded-full p-0.5 transition-colors duration-300 flex items-center ${syncToCalendar ? 'bg-emerald-400 justify-end' : 'bg-slate-300 justify-start'}`}>
                ...
                <AnimatePresence>
                  {alarmTime && (
                    <motion.div
                      ...
                      className="flex flex-col gap-3.5 bg-slate-50 border border-slate-200/60 p-4 rounded-2xl overflow-hidden shadow-inner"
                      id="alarm-repeat-container"
                    >
                      {/* Repetition options */}
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between">
                          <label className="text-[11px] font-bold tracking-wider text-slate-500 uppercase">알람 반복 주기 설정</label>
                          {alarmRepeat !== 'none' && (
                            <span className="text-[10px] font-extrabold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-lg animate-pulse animate-duration-1000">
                              반복 활성화됨
                            </span>
                          )}
                        </div>
                            ...
                            {['일', '월', '화', '수', '목', '금', '토'].map((dayName, idx) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex-1 h-9 rounded-xl border flex items-center justify-center text-xs font-bold transition-all duration-200 ${
                                    isDaySelected
                                      ? 'bg-emerald-600 border-transparent text-white scale-[1.03] shadow-md shadow-emerald-600/10'
                                      : 'bg-white border-slate-200 text-slate-605 hover:bg-emerald-50/50 hover:border-emerald-300 hover:text-emerald-950 shadow-sm'
                                  }`}
                                  id={`weekdayBtn-${idx}`}
                                >
                                  {dayName}
                      ...
                      {alarmRepeat === 'monthly_day' && (
                        ...
                        >
                          ...
                          {isMonthlyDayPickerConfirmed ? (
                            <div className="flex items-center justify-between bg-emerald-50 border border-emerald-250 p-3 rounded-2xl" id="monthly-day-picker-confirmed-view">
                              ...
                              <button
                                ...
                                id="monthly-day-picker-change-btn"
                              >
                                변경
                              </button>
                            </div>
                                ...
                                {Array.from({ length: 31 }).map((_, i) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`h-7 rounded-lg border text-[10px] font-mono font-bold flex items-center justify-center transition-all duration-205 ${
                                        isDaySelected
                      ...
                      <div className="flex flex-col gap-2 border-t border-slate-200/50 pt-2.5" id="alarm-sound-picker">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">알람 벨소리 형식 선택</span>
                          <span className="text-[9px] text-emerald-600 font-bold">선택 시 미리듣기 재생 🎵</span>
                        </div>
                        <div className="grid grid-cols-4 gap-1.5">
                              ...
                              <button
                                ...
                                className={`rounded-xl border py-2 px-1 text-[10px] text-center font-extrabold tracking-tight transition-all duration-300 cursor-pointer ${
                                  isSoundSelected
                                    ...
                                    : 'bg-white border-slate-200 text-slate-600 hover:bg-emerald-50/20 hover:text-emerald-900 hover:border-emerald-200'
                                }`}
                                id={`sound-typeBtn-${soundType}`}
                              >
              ...
              <div className="flex flex-col gap-2 border-t border-slate-200/50 pt-4">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 rounded-xl border border-slate-200 bg-white/70 px-3.5 py-2 text-xs text-slate-800 placeholder-slate-400 outline-none transition-all duration-300 focus:border-emerald-400"
                    id="todo-input-subtask"
                  />
                  <button
                    type="button"
                    onClick={handleAddSubtask}
                    className="rounded-xl border border-slate-200 bg-white/70 hover:bg-slate-100 text-slate-600 hover:text-slate-900 px-3 flex items-center justify-center transition-all duration-300 hover:border-slate-300 shadow-sm"
                    id="todo-btn-add-subtask"
                  >
                    <Plus size={16} />
                  </button>
                </div>

                {/* Subtask items display */}
                {subtasks.length > 0 && (
                    ...
                    {subtasks.map((sub, i) => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-700 font-medium">{sub.text}</span>
                        </div>
                        <button
                          type="button"
                          onClick={() => handleRemoveSubtask(sub.id)}
                          className="text-slate-500 hover:text-rose-500 transition-colors p-1"
                          id={`temp-sub-del-${sub.id}`}
                        >
                          <Trash2 size={13} />
                        </button>
                      </motion.div>
              ...
              {/* Complete additions button */}
              <div className="flex gap-3 justify-end mt-2">
                <button
                  type="button"
                  onClick={() => setIsOpen(false)}
                  className="rounded-xl border border-slate-200 bg-transparent px-5 py-2.5 text-xs font-bold text-slate-505 hover:text-slate-800 transition-all duration-300"
                  id="add-todo-cancel-btn"
                >
                  취소
                </button>
                <button
                  type="submit"
                  disabled={!title.trim()}
                  className={`rounded-xl px-5 py-2.5 text-xs font-bold shadow-lg transition-all duration-300 ${
                    title.trim()
                      ? 'bg-gradient-to-r from-emerald-400 via-teal-350 to-green-400 text-white shadow-emerald-400/20 hover:scale-[1.02] hover:shadow-emerald-400/30'
